...
@scenario(
    "../features/core-loop.feature", "Maria reaches the final round and does not agree"
)
...
@scenario(
    ...
    "Maria has a secured offer but chooses not to accept on the final round",
)
...
def then_secured_not_auto_accepted(ctx: ScenarioContext) -> None:
    ...
    assert ctx.state == "No Deal", f"Expected 'No Deal', got '{ctx.state}'"
